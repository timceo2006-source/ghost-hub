...
local GRAVITY_DROP = workspace.Gravity
local PING_OFFSET = 0.04
				...
				if aimbotEnabled and aimActive then
					...
					if myPart then
						...
						if targetPart then
							...
							local timeToTarget = (dist / BULLET_SPEED) + PING_OFFSET
							local movePrediction = targetVelocity * timeToTarget
							local dropAmount = 0.5 * GRAVITY_DROP * (timeToTarget ^ 2)
							local dropPrediction = Vector3.new(0, dropAmount, 0)
							
							local predictedPos = targetPart.Position + movePrediction + dropPrediction
							Camera.CFrame = Camera.CFrame:Lerp(CFrame.new(Camera.CFrame.Position, predictedPos), 1)
						end
					end
				end
			end)
